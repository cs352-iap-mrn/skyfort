        ...
        foreach (var e in enemyList)
        {
            if (e.Key == Enemy.EnemyType.Base)
            {
                for (int i = 0; i < e.Value; i++)
                {
                    ...
                    EnemyInstance newEnemy = new EnemyInstance(new Enemy(30, 2, 3, 3, 10, 10, 10, 10, Enemy.EnemyType.Boss, Tower.ModelType.Base), baseEnemyFab, posList[UnityEngine.Random.Range(0, posList.Length)]);
